...
const ShopContextProvider = (props) => {
  ...
  const checkSession = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const res = await api.get("/api/user/seller/session");
      if (res.data.success) {
        setSellerData(res.data.seller || null);
        setIsLoggedIn(true);
        connectSocket();
        return true;
      }
  ...
  const login = useCallback(async (credentialsOrSeller) => {
    setActionLoading(true);

    try {
      if (
        typeof credentialsOrSeller === "object" &&
        credentialsOrSeller?.email &&
        !credentialsOrSeller?.password
      ) {
        setSellerData(credentialsOrSeller);
        setIsLoggedIn(true);
        connectSocket();
        return { success: true, seller: credentialsOrSeller };
      }

      const res = await api.post("/api/user/seller", credentialsOrSeller);
      if (!res.data.success) {
      ...
      handleApiError(error, "Invalid seller credentials");
  ...
  const logout = useCallback(async () => {
    ...
    try {
      await api.post("/api/user/seller/logout");
    } catch {
  ...
  const value = useMemo(
    () => ({
      ...
      actionLoading,
      sellerData,
      sellerId: sellerData?.id || sellerData?._id || "",
